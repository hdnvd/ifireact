...
class sas_statusList extends SweetComponent {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            ...
            canDelete:AccessManager.UserCan('sas','status',AccessManager.DELETE),
    ...
    LoadData(pageSize,page,sorted,filtered)
    {
        ...
        let url='/sas/status?pg='+page+filterString;
    ...
    render(){
        return <div className={'pagecontent'}>
            ...
            <div className={'topoperationsrow'}><Link className={'addlink'}  to={'/sas/statuss/management'}><IoMdAddCircle/></Link></div>
        <SweetTable
            filterable={false}
...
{
    Header: 'به اتمام رسیده',
    id: 'is_commited',
    accessor: data=>data.commited==0?'خیر':'بله',
},
{
    Header: 'اتمام موفق',
    id: 'is_successful',
    accessor: data=>data.successful==0?'خیر':'بله',
},
{
    Header: 'عملیات',
    accessor: 'id',
    Cell: props => <div className={'operationsrow'}>
                   {!this.state.canEdit &&
                    <Link className={'viewlink'}  to={'/sas/statuss/management/'+props.value}><IoMdEye/></Link>
                   }
                   {this.state.canEdit &&
                    <Link className={'editlink'}  to={'/sas/statuss/management/'+props.value}><FaEdit/></Link>
                   }
                   {this.state.canDelete &&
                       <MdDeleteForever onClick={ 
                       () =>{
                         SweetAlert.displayDeleteAlert(()=>{
                            new SweetFetcher().Fetch('/sas/status/' + props.value, SweetFetcher.METHOD_DELETE, null,
                                data => {
                                    this.LoadData(Constants.DefaultPageSize,this.state.page+1,null,null);
                                        ...
                                        },'sas.status',AccessManager.DELETE,this.props.history);
                         });
                        }
                        }/>
                 }
