...
class sas_requestunittrackList extends SweetComponent {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            ...
            canDelete:AccessManager.UserCan('sas','requestunittrack',AccessManager.DELETE),
    ...
    LoadData(pageSize,page,sorted,filtered)
    {
        ...
        let url='/sas/requestunittrack?pg='+page+filterString;
        ...
        null,'sas.requestunittrack',AccessManager.LIST,
        this.props.history);
...
new SweetFetcher().Fetch('/sas/user',SweetFetcher.METHOD_GET,null,
    ...
    render(){
        return <div className={'pagecontent'}>
            ...
            <div className={'topoperationsrow'}><Link className={'addlink'}  to={'/sas/requestunittracks/management'}><IoMdAddCircle/></Link></div>
        <SweetTable
            filterable={false}
...
},
{
    ...
    Cell: props => <div className={'operationsrow'}>
                   {!this.state.canEdit &&
                    <Link className={'viewlink'}  to={'/sas/requestunittracks/management/'+props.value}><IoMdEye/></Link>
                   }
                   {this.state.canEdit &&
                    <Link className={'editlink'}  to={'/sas/requestunittracks/management/'+props.value}><FaEdit/></Link>
                   }
                   {this.state.canDelete &&
                       <MdDeleteForever onClick={ 
                       () =>{
                         SweetAlert.displayDeleteAlert(()=>{
                            new SweetFetcher().Fetch('/sas/requestunittrack/' + props.value, SweetFetcher.METHOD_DELETE, null,
                                data => {
                                    this.LoadData(Constants.DefaultPageSize,this.state.page+1,null,null);
                                        ...
                                        },'sas.requestunittrack',AccessManager.DELETE,this.props.history);
                         });
                        }
                        }/>
                 }
...
export default sas_requestunittrackList;
