...
class trapp_optionList extends SweetComponent {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            ...
            canInsert:AccessManager.UserCan('trapp','option',AccessManager.INSERT),
            canDelete:AccessManager.UserCan('trapp','option',AccessManager.DELETE),
    ...
    LoadData(pageSize,page,sorted,filtered)
    {
        ...
        let url='/trapp/option'+filterAndSortString;
    ...
    render(){
        return <div className={'pagecontent'}>
            ...
            <div className={'topoperationsrow'}>{this.state.canInsert && <Link className={'addlink'}  to={'/trapp/options/management'}><IoMdAddCircle/></Link>}</div>
        <SweetTable
            filterable={false}
...
//     Header: 'is_free',
//     id: 'free',
//     accessor: data=>data.free==0?'خیر':'بله',
// },
// {
//     Header: 'is_countable',
//     id: 'countable',
//     accessor: data=>data.countable==0?'خیر':'بله',
// },
{
    Header: 'عملیات',
    accessor: 'id',
    Cell: props => <div className={'operationsrow'}>
                   {!this.state.canEdit &&
                    <Link className={'viewlink'}  to={'/trapp/options/view/'+props.value}><IoMdEye/></Link>
                   }
                   {this.state.canEdit &&
                    <Link className={'editlink'}  to={'/trapp/options/management/'+props.value}><FaEdit/></Link>
                   }
                   {this.state.canDelete &&
                       <MdDeleteForever onClick={
                       () =>{
                         SweetAlert.displayDeleteAlert(()=>{
                            new SweetFetcher().Fetch('/trapp/option/' + props.value, SweetFetcher.METHOD_DELETE, null,
                                data => {
                                    this.LoadData(Constants.DefaultPageSize,this.state.page+1,null,null);
                                        ...
                                        },'trapp.option',AccessManager.DELETE,this.props.history);
                         });
                        }
                        }/>
                 }
...
export default trapp_optionList;
