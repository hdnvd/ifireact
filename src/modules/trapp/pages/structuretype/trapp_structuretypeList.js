...
class trapp_structuretypeList extends SweetComponent {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            pages:1,
            page:0,
            canEdit:AccessManager.UserCan('trapp','structuretype',AccessManager.EDIT),
            canInsert:AccessManager.UserCan('trapp','structuretype',AccessManager.INSERT),
            canDelete:AccessManager.UserCan('trapp','structuretype',AccessManager.DELETE),
    ...
    LoadData(pageSize,page,sorted,filtered)
    {
        ...
        let url='/trapp/structuretype'+filterAndSortString;
    ...
    render(){
        return <div className={'pagecontent'}>
            ...
            <div className={'topoperationsrow'}>{this.state.canInsert && <Link className={'addlink'}  to={'/trapp/structuretypes/management'}><IoMdAddCircle/></Link>}</div>
        <SweetTable
            filterable={false}
...
},
{
    ...
    Cell: props => <div className={'operationsrow'}>
                   {!this.state.canEdit &&
                    <Link className={'viewlink'}  to={'/trapp/structuretypes/view/'+props.value}><IoMdEye/></Link>
                   }
                   {this.state.canEdit &&
                    <Link className={'editlink'}  to={'/trapp/structuretypes/management/'+props.value}><FaEdit/></Link>
                   }
                   {this.state.canDelete &&
                       <MdDeleteForever onClick={ 
                       () =>{
                         SweetAlert.displayDeleteAlert(()=>{
                            new SweetFetcher().Fetch('/trapp/structuretype/' + props.value, SweetFetcher.METHOD_DELETE, null,
                                data => {
                                    this.LoadData(Constants.DefaultPageSize,this.state.page+1,null,null);
                                        ...
                                        },'trapp.structuretype',AccessManager.DELETE,this.props.history);
                         });
                        }
                        }/>
                 }
...
export default trapp_structuretypeList;
