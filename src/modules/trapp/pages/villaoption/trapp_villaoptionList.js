...
class trapp_villaoptionList extends SweetComponent {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            ...
            canInsert:AccessManager.UserCan('trapp','villaoption',AccessManager.INSERT),
            canDelete:AccessManager.UserCan('trapp','villaoption',AccessManager.DELETE),
    ...
    LoadData(pageSize,page,sorted,filtered)
    {
        ...
        let url='/trapp/villaoption'+filterAndSortString;
        ...
        null,'trapp.villaoption',AccessManager.LIST,
        this.props.history);
...
new SweetFetcher().Fetch('/trapp/option',SweetFetcher.METHOD_GET,null,
    ...
    render(){
        return <div className={'pagecontent'}>
                ...
                <MDBModal isOpen={this.state.displaySearchWindow} toggle={this.toggleSearchWindow}>
                    ...
                    <div className='form-group'>
                        ...
                        <select 
                                ...
                                onChange={(event)=>{this.searchParams.villa=event.target.value;}}>
                    ...
                    <div className='form-group'>
                        ...
                        <select 
                                id='option'
                                className='browser-default custom-select'
                                onChange={(event)=>{this.searchParams.option=event.target.value;}}>
                                <option value={''}>همه</option>
            ...
            <div className={'topoperationsrow'}>{this.state.canInsert && <Link className={'addlink'}  to={'/trapp/villaoptions/management'}><IoMdAddCircle/></Link>}</div>
        <SweetTable
            filterable={false}
...
columns = [
{
    Header: 'ویلا',
    id: 'villa',
    accessor: data=>data.villacontent,
},
{
    Header: 'option_fid',
    id: 'option',
    accessor: data=>data.optioncontent,
},
{
    Header: 'عملیات',
    accessor: 'id',
    Cell: props => <div className={'operationsrow'}>
                   {!this.state.canEdit &&
                    <Link className={'viewlink'}  to={'/trapp/villaoptions/view/'+props.value}><IoMdEye/></Link>
                   }
                   {this.state.canEdit &&
                    <Link className={'editlink'}  to={'/trapp/villaoptions/management/'+props.value}><FaEdit/></Link>
                   }
                   {this.state.canDelete &&
                       <MdDeleteForever onClick={ 
                       () =>{
                         SweetAlert.displayDeleteAlert(()=>{
                            new SweetFetcher().Fetch('/trapp/villaoption/' + props.value, SweetFetcher.METHOD_DELETE, null,
                                data => {
                                    this.LoadData(Constants.DefaultPageSize,this.state.page+1,null,null);
                                        ...
                                        },'trapp.villaoption',AccessManager.DELETE,this.props.history);
                         });
                        }
                        }/>
                 }
...
export default trapp_villaoptionList;
