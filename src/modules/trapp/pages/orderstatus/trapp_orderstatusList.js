...
class trapp_orderstatusList extends SweetComponent {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            pages:1,
            page:0,
            canEdit:AccessManager.UserCan('trapp','orderstatus',AccessManager.EDIT),
            canInsert:AccessManager.UserCan('trapp','orderstatus',AccessManager.INSERT),
            canDelete:AccessManager.UserCan('trapp','orderstatus',AccessManager.DELETE),
    ...
    LoadData(pageSize,page,sorted,filtered)
    {
        ...
        let url='/trapp/orderstatus'+filterAndSortString;
    ...
    render(){
        return <div className={'pagecontent'}>
            ...
            <div className={'topoperationsrow'}>{this.state.canInsert && <Link className={'addlink'}  to={'/trapp/orderstatuss/management'}><IoMdAddCircle/></Link>}</div>
        <SweetTable
            filterable={false}
...
},
{
    ...
    Cell: props => <div className={'operationsrow'}>
                   {!this.state.canEdit &&
                    <Link className={'viewlink'}  to={'/trapp/orderstatuss/view/'+props.value}><IoMdEye/></Link>
                   }
                   {this.state.canEdit &&
                    <Link className={'editlink'}  to={'/trapp/orderstatuss/management/'+props.value}><FaEdit/></Link>
                   }
                   {this.state.canDelete &&
                       <MdDeleteForever onClick={ 
                       () =>{
                         SweetAlert.displayDeleteAlert(()=>{
                            new SweetFetcher().Fetch('/trapp/orderstatus/' + props.value, SweetFetcher.METHOD_DELETE, null,
                                data => {
                                    this.LoadData(Constants.DefaultPageSize,this.state.page+1,null,null);
                                        ...
                                        },'trapp.orderstatus',AccessManager.DELETE,this.props.history);
                         });
                        }
                        }/>
                 }
...
export default trapp_orderstatusList;
