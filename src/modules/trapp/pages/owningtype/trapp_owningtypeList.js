...
class trapp_owningtypeList extends SweetComponent {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            pages:1,
            page:0,
            canEdit:AccessManager.UserCan('trapp','owningtype',AccessManager.EDIT),
            canInsert:AccessManager.UserCan('trapp','owningtype',AccessManager.INSERT),
            canDelete:AccessManager.UserCan('trapp','owningtype',AccessManager.DELETE),
    ...
    LoadData(pageSize,page,sorted,filtered)
    {
        ...
        let url='/trapp/owningtype'+filterAndSortString;
    ...
    render(){
        return <div className={'pagecontent'}>
            ...
            <div className={'topoperationsrow'}>{this.state.canInsert && <Link className={'addlink'}  to={'/trapp/owningtypes/management'}><IoMdAddCircle/></Link>}</div>
        <SweetTable
            filterable={false}
...
},
{
    ...
    Cell: props => <div className={'operationsrow'}>
                   {!this.state.canEdit &&
                    <Link className={'viewlink'}  to={'/trapp/owningtypes/view/'+props.value}><IoMdEye/></Link>
                   }
                   {this.state.canEdit &&
                    <Link className={'editlink'}  to={'/trapp/owningtypes/management/'+props.value}><FaEdit/></Link>
                   }
                   {this.state.canDelete &&
                       <MdDeleteForever onClick={ 
                       () =>{
                         SweetAlert.displayDeleteAlert(()=>{
                            new SweetFetcher().Fetch('/trapp/owningtype/' + props.value, SweetFetcher.METHOD_DELETE, null,
                                data => {
                                    this.LoadData(Constants.DefaultPageSize,this.state.page+1,null,null);
                                        ...
                                        },'trapp.owningtype',AccessManager.DELETE,this.props.history);
                         });
                        }
                        }/>
                 }
...
export default trapp_owningtypeList;
