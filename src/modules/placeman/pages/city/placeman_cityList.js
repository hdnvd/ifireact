...
class placeman_cityList extends SweetComponent {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            ...
            canInsert:AccessManager.UserCan('placeman','city',AccessManager.INSERT),
            canDelete:AccessManager.UserCan('placeman','city',AccessManager.DELETE),
    ...
    LoadData(pageSize,page,sorted,filtered)
    {
        ...
        let url='/placeman/city'+filterAndSortString;
...
new SweetFetcher().Fetch('/placeman/province',SweetFetcher.METHOD_GET,null,
                data=>{
    ...
    render(){
        return <div className={'pagecontent'}>
                ...
                <MDBModal isOpen={this.state.displaySearchWindow} toggle={this.toggleSearchWindow}>
                    ...
                    <div className='form-group'>
                        ...
                        <select
                                ...
                                onChange={(event)=>{this.searchParams.province=event.target.value;}}>
            ...
            <div className={'topoperationsrow'}>{this.state.canInsert && <Link className={'addlink'}  to={'/placeman/citys/management'}><IoMdAddCircle/></Link>}</div>
        <SweetTable
            filterable={false}
...
},
{
    Header: 'استان',
    id: 'province',
    accessor: data=>data.provincecontent,
},
{
    Header: 'عملیات',
    accessor: 'id',
    Cell: props => <div className={'operationsrow'}>
                   {!this.state.canEdit &&
                    <Link className={'viewlink'}  to={'/placeman/citys/view/'+props.value}><IoMdEye/></Link>
                   }
                   {this.state.canEdit &&
                    <Link className={'editlink'}  to={'/placeman/citys/management/'+props.value}><FaEdit/></Link>
                   }
                   {this.state.canDelete &&
                       <MdDeleteForever onClick={
                       () =>{
                         SweetAlert.displayDeleteAlert(()=>{
                            new SweetFetcher().Fetch('/placeman/city/' + props.value, SweetFetcher.METHOD_DELETE, null,
                                data => {
                                    this.LoadData(Constants.DefaultPageSize,this.state.page+1,null,null);
                                        ...
                                        },'placeman.city',AccessManager.DELETE,this.props.history);
                         });
                        }
                        }/>
                 }
...
export default placeman_cityList;
